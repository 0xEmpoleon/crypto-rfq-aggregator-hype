...
import type { CellData, HoverTip, MetaTip } from '../types';
...
export const MatrixCell = memo(function MatrixCell({
    ...
    pinnedPos, dbitPrice, darkMode, assetSymbol, priceSource, onHover, onTogglePin, onHoverMeta,
}: MatrixCellProps) {
    if (!d) {
    ...
    return (
        <td onClick={(e) => onTogglePin(k, e.clientX, e.clientY)}
            ...
            <div style={{ position: 'relative', height: '100%', zIndex: isP ? 999 : (isRec ? 2 : 1), display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
